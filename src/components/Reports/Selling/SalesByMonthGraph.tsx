  ...
  const [type, setType] = useState<SalesByMonthGraphTypes>(SalesByMonthGraphTypes.TOTAL);
    ...
    if(data){
      [...new Set(data.map(r => r.year))].forEach(year => {
        graphData.push({
          id: String(year), 
          data: data.filter(r => r.year == year).map(r => ({ x: receiveNameOfMonthByNumberOfMonth(r.month, 'long'), y: type == SalesByMonthGraphTypes.TOTAL ? r.total : r.pairs })),
        });
      })
    }

    return <Box width={"100%"} display={"flex"}  flexDirection={"column"} height={300} p={2} alignItems={"flex-start"} >
        ...
        <div style={{ height: '400px', width:"100%" }}>
            <ResponsiveLine
                    data={graphData}
                    margin={{ top:30, right: 30, bottom: 260, left: type == SalesByMonthGraphTypes.TOTAL ? 120 : 80 }}
                    animate
                    curve="monotoneX"
                    yScale={{
                      stacked: true,
